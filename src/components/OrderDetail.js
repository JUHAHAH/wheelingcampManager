    ...
    Cell: ({ renderedCellValue, row }) => (
      <Box
        sx={{
          display: 'flex',
          alignItems: 'center',
          backgroundColor: renderedCellValue == 'N' ? '#D2222DCC' : '#238823CC',
...
const OrderDetail = (props) => {
  ...
  useEffect(() => {
    setColumns(props.payCode == 1 ? purchaseColumn : rentColumn);

    axios
      .get(
        `https://wheelingcamp.store/manage/orderDetail?payCode=${props.payCode}&payNo=${props.payNo}`
      )
      .then((data) => {
  ...
  const handleSaveUser = async ({ values, table }) => {
    await axios
      .put('https://wheelingcamp.store/manage/updateOrderDetail', null, {
        params: values,
      })
      .then((result) => {
